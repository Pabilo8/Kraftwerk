...
public class GLAllocation
{
	...
	{
		int i = Kraftwerk.gl.glGenLists(range);

		if(i==0)
		{
			int j = Kraftwerk.gl.glGetError();
			String s = "No error code reported";

			if(j!=0)
			{
				s = Kraftwerk.INSTANCE.panelModelEditor.glu.gluErrorString(j);
			}
	...
	{
		Kraftwerk.gl.glDeleteLists(list, range);
	}
