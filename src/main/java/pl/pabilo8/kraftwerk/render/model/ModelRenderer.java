...
public class ModelRenderer
{
	...
	public ModelRenderer addBox(String partName, float offX, float offY, float offZ, int width, int height, int depth)
	{
		...
		Vec2i textureoffset = this.baseModel.getTextureOffset(partName);
	...
	public void render(float scale)
	{
		if(!this.isHidden)
		{
			if(this.showModel)
			{
				// TODO: 22.07.2021 fix, performance not stok
				if(!this.compiled)
				{
					this.compileDisplayList(scale);
				}

				Kraftwerk.gl.glTranslatef(this.offsetX, this.offsetY, this.offsetZ);

				if(this.rotateAngleX==0.0F&&this.rotateAngleY==0.0F&&this.rotateAngleZ==0.0F)
				{
					if(this.rotationPointX==0.0F&&this.rotationPointY==0.0F&&this.rotationPointZ==0.0F)
					{
						Kraftwerk.gl.glCallList(this.displayList);
					...
					{
						Kraftwerk.gl.glTranslatef(this.rotationPointX*scale, this.rotationPointY*scale, this.rotationPointZ*scale);
						Kraftwerk.gl.glCallList(this.displayList);
				...
				else
				{
					...
					{
						Kraftwerk.gl.glRotatef(this.rotateAngleZ*(180F/(float)Math.PI), 0.0F, 0.0F, 1.0F);
					}

					if(this.rotateAngleY!=0.0F)
					{
						Kraftwerk.gl.glRotatef(this.rotateAngleY*(180F/(float)Math.PI), 0.0F, 1.0F, 0.0F);
					}

					if(this.rotateAngleX!=0.0F)
					{
						Kraftwerk.gl.glRotatef(this.rotateAngleX*(180F/(float)Math.PI), 1.0F, 0.0F, 0.0F);
					}
	...
	private void compileDisplayList(float scale)
	{
		...
		Kraftwerk.gl.glNewList(this.displayList, 4864);
		//BufferBuilder bufferbuilder = Tessellator.getInstance().getBuffer();

		for(int i = 0; i < this.cubeList.size(); ++i)
		{
			this.cubeList.get(i).render(scale);
		}

		Kraftwerk.gl.glEndList();
		this.compiled = true;
	}
